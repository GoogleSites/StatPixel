...
import type { MurderMysteryData, Message, MessageOptions } from '../../typings';

export default class MurderMystery extends BaseCommand {
  constructor(id: string, client: Main) {
    super(id, client);

    this.aliases.push('mm');
    this.arguments = [
      new Argument(
        'username',
  ...
  public async run(
    ...
    template
      ...
      .text('MURDER MYSTERY // OVERALL', 305, 100, {
        colour: 'tertiary',
        size: 30,
        parseColours: false
      })
    ...
    if (guild !== null && guild.tag) {
      const labelWidth = template.measure('MURDER MYSTERY // OVERALL', { size: 30 });
    ...
    const generate = async (labels: string[], keys: (keyof MurderMysteryData)[]) => {
    ...
    const images = await Promise.all([
      generate(
        ['General', 'Classic', 'Double Up'],
        ['overall', 'classic', 'double_up']
      ),
      generate(
        ['Assassins', 'Showdown', 'Infection'],
        ['assassins', 'showdown', 'infection']
      )
    ]);
