...
import type { SpeedUHCData, Message, MessageOptions } from '../../typings';

export default class SpeedUHC extends BaseCommand {
  constructor(id: string, client: Main) {
    super(id, client);

    this.aliases.push('suhc');
    this.arguments = [
      new Argument(
        'username',
  ...
  public async run(
    ...
    template
      ...
      .text('SPEED UHC // OVERALL', 305, 100, {
        colour: 'tertiary',
        size: 30,
        parseColours: false
      })
      ...
      .text(`§fSalt: ${this.client.util.formatNumber(data.overall.salt)}`, 316, 301, { size: 36, minecraftChatColours: true, font: 'Segoe UI Bold' });
    ...
    if (guild !== null && guild.tag) {
      const labelWidth = template.measure('SPEEDUHC // OVERALL', { size: 30 });
    ...
    const generate = async (labels: string[], keys: (keyof SpeedUHCData)[]) => {
    ...
    const images = await Promise.all([
      ...
      generate(
        ['Solo Insane', 'Teams Insane'],
        ['solo_insane', 'team_insane']
      )
    ]);
