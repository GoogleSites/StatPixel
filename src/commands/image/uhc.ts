...
import type { UHCData, Message, MessageOptions } from '../../typings';
...
export default class UHC extends BaseCommand {
  ...
  public async run(
    ...
    if (guild !== null && guild.tag) {
      const labelWidth = template.measure('UHC // OVERALL', { size: 30 });
...

    {
      ...
      const coinsWidth = template.measure(`Coins: ${coins}`, { size: 36, font: 'Segoe UI Bold' });

      template
        .text(`§fCoins: §6${coins}`, 316, 301, { size: 36, minecraftChatColours: true, font: 'Segoe UI Bold' })
        .image(this.client.profile.images['coin.svg'], 321 + coinsWidth, 270, 36, 36)
        .text(`§fScore: ${this.client.util.formatNumber(data.overall.score)}`, 316 + coinsWidth + 100, 301, { size: 36, minecraftChatColours: true, font: 'Segoe UI Bold' });
    }
    ...
    const generate = async (labels: string[], keys: (keyof UHCData)[]) => {
    ...
    const images = await Promise.all([
      ...
      generate(
        ['Teams of 3', 'Solo Brawl'],
        ['three_team', 'solo_brawl']
      )
    ]);
