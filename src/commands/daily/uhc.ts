...
export default class UHC extends BaseCommand {
  constructor(id: string, client: Main) {
    ...
    this.description = 'Retrieves the daily UHC session statistics of a player';
  }
  ...
  public async run(
    ...
    const { id, created_at: started } =
      await this.client.profile.findSessionOfType('daily', player);
...
Score: **${this.client.util.formatDifference(
      ...
      maxFields: 3,
      title: {
        name: `${this.client.hutil.computeDisplayName(player)} ➢ UHC (Daily)`,
